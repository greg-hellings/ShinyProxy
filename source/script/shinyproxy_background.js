...
var envinfo = {"qx.application":"shinyproxy.Background","qx.debug":false,"qx.optimization.basecalls":true,"qx.optimization.comments":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.variants":true,"qx.revision":"exported","qx.theme":"shinyproxy.theme.Theme","qx.version":"1.5"};
for (var k in envinfo) qx.$$environment[k] = envinfo[k];

if (!qx.$$libraries) qx.$$libraries = {};
var libinfo = {"__out__":{"sourceUri":"script"},"chrome":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"dialog":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"mutablelist":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.5"},"shinyproxy":{"resourceUri":"resource","sourceUri":"script","version":"trunk"}};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:shinyproxy_background.1ddadeade52e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="os.name",j="os.version",h="css.borderradius",g="default",f="browser.quirksmode",e="browser.name",d="qx.allowUrlSettings",c="event.pointer",b="io.ssl",a="locale.variant",V="css.textoverflow",U="html.xul",T="css.boxshadow",S="event.touch",R="css.gradients",Q="browser.documentmode",P="ecmascript.objectcount",O="locale",N="engine.version",M="engine.name",r="css.rgba",s="css.boxmodel",p="|",q="browser.version",n="io.xhr",o="qx.core.Environment",l="qx.debug.databinding",m="qx.optimization.basecalls",t="qx.debug.dispose",u="qx.optimization.variables",B="true",z="qx.optimization.privates",F="qx.aspects",D="qx.debug",I="qx.dynamicmousewheel",H=":",w="qxenv",L="qx.optimization.strings",K="qx.optimization.comments",J="qx.optimization.variants",v="qx.mobile.emulatetouch",x="qx.dynlocale",y="false",A="&",C="qx.mobile.nativescroll",E="qx.allowUrlVariants",G="qx.propertyDebugLevel";
qx.Bootstrap.define(o,{statics:{_checks:{},_asyncChecks:{},__j:{},get:function(W){if(this.__j[W]!=undefined){return this.__j[W];
}var X=this._checks[W];

if(X){var Y=X();
this.__j[W]=Y;
return Y;
}if(qx.Bootstrap.DEBUG){qx.Bootstrap.warn(W+" is not a valid key. Please see the API-doc of "+"qx.core.Environment for a list of predefined keys.");
qx.Bootstrap.trace(this);
}},getAsync:function(ba,bb,self){var bd=this;

if(this.__j[ba]!=undefined){window.setTimeout(function(){bb.call(self,bd.__j[ba]);
},0);
return;
}var bc=this._asyncChecks[ba];

if(bc){bc(function(be){bd.__j[ba]=be;
bb.call(self,be);
});
return;
...
qx.Bootstrap.trace(this);
}},select:function(bf,bg){return this.__k(this.get(bf),bg);
},selectAsync:function(bh,bi,self){this.getAsync(bh,function(bj){var bk=this.__k(bh,bi);
bk.call(self,bj);
},this);
},__k:function(bl,bm){var bo=bm[bl];

if(bm.hasOwnProperty(bl)){return bo;
}for(var bn in bm){if(bn.indexOf(p)!=-1){var bp=bn.split(p);

for(var i=0;i<bp.length;i++){if(bp[i]==bl){return bm[bn];
}}}}
if(bm[g]!==undefined){return bm[g];
}
if(qx.Bootstrap.DEBUG){throw new Error('No match for variant "'+bl+'" ('+(typeof bl)+' type)'+' in variants ['+qx.Bootstrap.getKeysAsString(bm)+'] found, and no default ("default") given');
}},invalidateCacheKey:function(bq){delete this.__j[bq];
},add:function(br,bs){if(this._checks[br]==undefined){if(bs instanceof Function){this._checks[br]=bs;
}else{this._checks[br]=this.__n(bs);
}}},addAsync:function(bt,bu){if(this._checks[bt]==undefined){this._asyncChecks[bt]=bu;
}},_initDefaultQxValues:function(){this.add(d,function(){return false;
});
this.add(E,function(){return false;
});
this.add(G,function(){return 0;
});
...
},__l:function(){if(qx&&qx.$$environment){for(var bw in qx.$$environment){var bv=qx.$$environment[bw];
this._checks[bw]=this.__n(bv);
}}},__m:function(){if(window.document&&window.document.location){var bx=window.document.location.search.slice(1).split(A);

for(var i=0;i<bx.length;i++){var bz=bx[i].split(H);

if(bz.length!=3||bz[0]!=w){continue;
}var bA=bz[1];
var by=decodeURIComponent(bz[2]);
if(by==B){by=true;
}else if(by==y){by=false;
}else if(/^(\d|\.)+$/.test(by)){by=parseFloat(by);
}this._checks[bA]=this.__n(by);
}}},__n:function(bB){return qx.Bootstrap.bind(function(bC){return bC;
},null,bB);
},useCheck:function(bD){return true;
},_initChecksMap:function(){if(this.useCheck(N)){this._checks[N]=qx.bom.client.Engine.getVersion;
}
if(this.useCheck(M)){this._checks[M]=qx.bom.client.Engine.getName;
...
if(this.useCheck(n)){this._checks[n]=qx.bom.client.Transport.getXmlHttpRequest;
}if(this.useCheck(S)){this._checks[S]=qx.bom.client.Event.getTouch;
}
...
if(this.useCheck(U)){this._checks[U]=qx.bom.client.Html.getXul;
}if(this.useCheck(V)){this._checks[V]=qx.bom.client.Css.getTextOverflow;
}
if(this.useCheck(h)){this._checks[h]=qx.bom.client.Css.getBorderRadius;
}
if(this.useCheck(T)){this._checks[T]=qx.bom.client.Css.getBoxShadow;
}
...
}}},defer:function(bE){bE._initDefaultQxValues();
bE._initChecksMap();
bE.__l();
if(bE.get(d)===true){bE.__m();
}}});
})();
...
(function(){var a="shinyproxy.theme.Font";
...
}},__sg:function(G){var J=this._generateBackgroundMarkup(G);
...
G[y]=0;
G[p]=0;
return this.__se=this._generateBackgroundMarkup(G,J);
},__sh:function(K,L,M){var N=this.getInsets();
L-=N.left+N.right;
M-=N.top+N.bottom;
...
},__si:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
...
}},members:{__sj:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__sj;
},getMarkup:function(e){if(this.__sj){return this.__sj;
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__sj:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__sj;
},_applyStyle:function(){},getMarkup:function(){if(this.__sj){return this.__sj;
...
if(p){this.__sl(p);
}}else{this.__sk=new qx.ui.decoration.GridDiv(p);
}
if(q!=null){this.__sk.setInsets(q);
}},properties:{baseImage:{check:d,nullable:true,apply:j},insetLeft:{check:o,nullable:true,apply:n},insetRight:{check:o,nullable:true,apply:n},insetBottom:{check:o,nullable:true,apply:n},insetTop:{check:o,nullable:true,apply:n},insets:{group:[k,l,i,e],mode:f}},members:{__sk:null,getMarkup:function(){return this.__sk.getMarkup();
},resize:function(r,s,t){this.__sk.resize(r,s,t);
},tint:function(u,v){},getInsets:function(){return this.__sk.getInsets();
},_applyInsets:function(w,x,name){var y=g+qx.lang.String.firstUp(name);
this.__sk[y](w);
},_applyBaseImage:function(z,A){if(this.__sk instanceof qx.ui.decoration.GridDiv){this.__sk.setBaseImage(z);
}else{this.__sl(z);
}},__sl:function(B){var G,H,J,I;
this.__sk.setBorderImage(B);
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__sj:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__sj;
},getMarkup:function(){if(this.__sj){return this.__sj;
...
this.__sj=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__sj;
...
(function(){var a="shinyproxy.theme.Decoration";
...
(function(){var a="shinyproxy.theme.Color";
...
(function(){var a="shinyproxy.theme.Appearance";
qx.Theme.define(a,{extend:qx.theme.modern.Appearance,appearances:{}});
})();
(function(){var a="shinyproxy.theme.Theme";
qx.Theme.define(a,{meta:{color:shinyproxy.theme.Color,decoration:shinyproxy.theme.Decoration,font:shinyproxy.theme.Font,icon:qx.theme.icon.Tango,appearance:shinyproxy.theme.Appearance}});
})();
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="os.name",b="engine.version",a="shinyproxy.Background";
...
qx.Class.define(a,{statics:{getApplication:qx.core.BaseInit.getApplication,ready:qx.core.BaseInit.ready,__cV:function(e){var f=this.__application;

if(f){e.setReturnValue(f.close());
}},__cW:function(){var g=this.__application;
...
this._initObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{orientationchange:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__eI:null,__cu:null,__fr:null,__fs:null,__ft:null,canHandleEvent:function(g,h){},registerEvent:function(i,j,k){},unregisterEvent:function(l,m,n){},_initObserver:function(){this.__ft=qx.lang.Function.listener(this._onNative,this);
this.__fr=qx.bom.Event.supportsEvent(this.__cu,e)?e:d;
var Event=qx.bom.Event;
Event.addNativeListener(this.__cu,this.__fr,this.__ft);
},_stopObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__cu,this.__fr,this.__ft);
...
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{useraction:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__eI:null,__cu:null,canHandleEvent:function(c,d){},registerEvent:function(e,f,g){},unregisterEvent:function(h,i,j){}},destruct:function(){this.__eI=this.__cu=null;
...
this.__eI=this.__cu=this.__cX=this.__fD=null;
...
},__fX:function(){var H=[this.__cu,this.__cX,this.__cX.body];
var I=this.__cu;
var G=h;
...
this.__eI=this.__cu=this.__cX=this.__fW=null;
...
}}},members:{__gj:null,__eI:null,__cu:null,__cX:null,__gi:null,__gk:null,__gl:null,__gm:null,canHandleEvent:function(bB,bC){},registerEvent:function(bD,bE,bF){},unregisterEvent:function(bG,bH,bI){},_fireInputEvent:function(bJ,bK){var bL=this.__gn();
...
this.__gk=this.__eI=this.__cu=this.__cX=this.__gi=null;
...
},statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST},members:{__cw:null,__ho:null,__hp:true,__cu:null,_getParent:function(l){return l.parentNode;
...
},"default":qx.lang.Function.empty})},destruct:function(){this.__ho=this.__cu=this.__cw=null;
...
},members:{__co:null,__hx:null,getId:function(){return this.__co;
...
},members:{__cu:null,__kA:null,_createContainerElement:function(){var p=this.__kA;
...
}}},destruct:function(){this.__cu=this.__kA=null;
}});
})();
...
(function(){var g="External IP: ",f='GET',d="Set",c="shinyproxy.Background",b='http://dumpr.info/api/1.0/remote-addr',a="success";
...
var h=new shinyproxy.backend.ProxyServiceImpl();
this.__sm=new qx.io.request.Xhr(b,f);
this.__sm.set({cache:false});
this.__sm.addListener(a,this.__so,this);
qx.lang.Function.periodical(this.__sn,300000,this);
chrome.proxy.settings.onChange.addListener(qx.lang.Function.listener(this.__sn,this));
this.__sn();
},__sn:function(){this.__sm.send();
},__so:function(e){if(e.getTarget){try{var i=qx.lang.Json.parse(e.getTarget().getResponse());
chrome.browserAction.setTitle({title:g+i.ip});
console.log(d);
}catch(e){}}}}});
})();
(function(){var f='undefined',e="chromeplugin.backend.ServiceImpl",d="abstract",c="BackgroundServiceImpl::register caught an attempt to register an invalid value as a function",b="Invalid type passed for BackgroundServiceImpl::dispatch's argument array",a="Invalid endpoint registered for method name ";
qx.Class.define(e,{extend:qx.core.Object,type:d,construct:function(g){var h=chromeplugin.backend.Dispatcher.getInstance();
h.register(g,this);
this.__sp=new Array();
},members:{__sp:null,dispatch:function(i,j){if(!qx.lang.Type.isArray(j)&&typeof (j)!=f&&j!=null){this.error("Invalid type passed for BackgroundServiceImpl::dispatch's argument array");
throw b;
return;
}
try{var k=this.__sp[i];
if(qx.lang.Type.isFunction(k)){if(typeof (j)==f||j==null)var l=k.call(this);
else var l=k.apply(this,j);
return l;
}else{throw a+i;
}}catch(m){this.error("Invalid endpoint registered for method name "+i+" or other error:\n"+m);
}},register:function(n,o){if(!qx.lang.Type.isFunction(o)){this.error("Non-function being registered in BackgroundServiceImpl");
throw c;
return;
}if(n===null||typeof (n)==f){this.debug("Attempt to register a function with no endpoint set");
}this.__sp[n]=o;
}}});
})();
(function(){var f='undefined',e="chromeplugin.backend.Dispatcher",d="qxchrome::backend::Dispatcher::register has detected an attempt to register an invalid service provider for ",c="qxchrome::backend::Dispatcher::register has detected an attempt to register an invalid serviceName or undefined service",b="qxchrome::backend::Dispatcher::dispatch has detected an invalid service endpoint",a="singleton";
qx.Class.define(e,{extend:qx.core.Object,type:a,defer:function(g,h,i){var j=chromeplugin.backend.Dispatcher.getInstance();
},construct:function(){this.__sq=new Array();
chrome.extension.onRequest.addListener(qx.lang.Function.bind(this.dispatch,this));
},members:{__sq:null,dispatch:function(k,l,m){var q=this.__sq[k.service];
if(typeof (q)==f||!qx.lang.Type.isObject(q)){this.error("Service endpoint does not exist or is undefined or not an object.\n"+"  Requested service is "+k.service+" and I have "+q);
throw b;
}var p=k.method;
var o=k.args;
var n=q.dispatch(p,o);
m(n);
},register:function(r,s){if(!qx.lang.Type.isObject(s)){this.error("Attempt to set a non-object as the service provider for "+r);
throw d+r;
return;
}
if(typeof (r)==f||typeof (s)==f){this.error("Attempt to set an invalid serviceName or undefined service");
throw c;
return;
}this.__sq[r]=s;
}}});
...
}else{if(m.version!=this.__ss){m=this.__st(m);
}this.__sr=m.entries;
}},members:{__sr:[],__ss:1,list:function(){return this.__sr;
...
},get:function(q){if(q<this.__sr.length)return this.__sr[q];
else return null;
},save:function(r,s){s.key=r;
var t=false;
this.__sr.forEach(function(u,v){if(u.key==r){this.__sr[v]=s;
t=true;
}},this);
if(!t)this.__sr.push(s);
this.__lt();
},clear:function(){this.__sr=[];
this.__lt();
},__lt:function(x){var w;

if(!x)w={version:this.__ss,entries:this.__sr};
else w=x;
localStorage.setItem(l,qx.lang.Json.stringify(w));
console.log(localStorage.proxies);
},__st:function(y){y={version:this.__ss,entries:[]};
this.__lt(y);
return y;
}}});
})();
...
this._setPhase(p);
this.__su=qx.lang.Function.bind(this._onReadyStateChange,this);
this.__sv=qx.lang.Function.bind(this._onLoad,this);
this.__sw=qx.lang.Function.bind(this._onLoadEnd,this);
this.__sx=qx.lang.Function.bind(this._onAbort,this);
this.__sy=qx.lang.Function.bind(this._onTimeout,this);
this.__sz=qx.lang.Function.bind(this._onError,this);
E.onreadystatechange=this.__su;
E.onload=this.__sv;
E.onloadend=this.__sw;
E.onabort=this.__sx;
E.ontimeout=this.__sy;
E.onerror=this.__sz;
...
},nullable:true},authentication:{check:B,nullable:true}},members:{__su:null,__sv:null,__sw:null,__sx:null,__sy:null,__sz:null,__sA:null,__sB:null,_transport:null,_createTransport:function(){throw new Error("Abstract method call");
...
},getPhase:function(){return this.__sB;
...
if(this.__sA!==M){this.__sA=M;
this.fireEvent(m,qx.event.type.Data,[this.__sA,N]);
...
this.fireEvent(Q);
},_setPhase:function(R){var S=this.__sB;
this.__sB=R;
...
}},__sC:function(){var V=this.getRequestHeaders();

if(V){qx.lang.Object.getKeys(V).forEach(function(W){this._transport.setRequestHeader(W,V[W]);
},this);
}},__sD:function(){var Y=this.getAuthentication(),X=this._transport;
...
B+=this.__mP(name,z,w);
}}else if(qx.lang.Type.isDate(y)&&x!=null){B+=this.__mP(name,x.format(y),w);
}else{B+=this.__mP(name,y,w);
}}return B.substring(0,B.length-1);
},__mP:function(name,D,E){if(D instanceof qx.core.Object&&E!=null){var F=encodeURIComponent(E(D));
...
},init:true}},members:{__sE:null,_createTransport:function(){return new qx.bom.request.Xhr();
...
if(typeof D.PARSER[C]===r){return this.__sE=D.PARSER[C];
}return this.__sE=C;
},_getParser:function(){var E=this.__sE;
if(E){return E;
...
qx.Bootstrap.define(d,{construct:function(){this.__sF=qx.Bootstrap.bind(this.__sO,this);
this.__sy=qx.Bootstrap.bind(this.__sR,this);
this.__sN();
if(window.attachEvent){this.__sG=qx.Bootstrap.bind(this.__sU,this);
window.attachEvent(m,this.__sG);
}},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{readyState:0,responseText:r,responseXML:null,status:0,statusText:r,timeout:0,open:function(t,u,v,w,x){if(this.__nV){return;
}this.__sH=false;
this.__sI=false;
...
this.__sN();
}this.__sL.onreadystatechange=this.__sF;

try{if(qx.core.Environment.get(q)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Open native request with method: "+t+", url: "+u+", async: "+v);
}this.__sL.open(t,u,v,w,x);
}catch(y){if(!qx.util.Request.isCrossDomain(u)){throw y;
}
if(!this.__sJ){this.__sK=y;
}
if(this.__sJ){if(window.XDomainRequest){this.readyState=4;
this.__sL=new XDomainRequest();
this.__sL.onerror=qx.Bootstrap.bind(function(){this.onreadystatechange();
this.onerror();
this.onloadend();
},this);
...
}}if(qx.core.Environment.get(s)===o&&parseInt(qx.core.Environment.get(n),10)<2&&!this.__sJ){this.readyState=qx.bom.request.Xhr.OPENED;
this.onreadystatechange();
}},setRequestHeader:function(z,A){if(this.__nV){return;
}this.__sL.setRequestHeader(z,A);
},send:function(B){if(this.__nV){return;
}if(!this.__sJ&&this.__sK){throw this.__sK;
}B=typeof B==k?null:B;
try{if(qx.core.Environment.get(q)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Send native request");
}this.__sL.send(B);
}catch(C){if(!this.__sJ){throw C;
}}if(qx.core.Environment.get(s)===o&&!this.__sJ){this.__sO();
}this.__sH=true;
if(qx.core.Environment.get(s)===l&&this.timeout===0){this.timeout=10000;
}if(this.timeout>0){this.__sM=window.setTimeout(this.__sy,this.timeout);
}},abort:function(D){if(this.__nV){return;
}this.__sI=true;
this.__sL.abort();

if(this.__sL){this.readyState=this.__sL.readyState;
}
if(D){return;
}this.onabort();
},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(E){if(this.__nV){return;
}return this.__sL.getResponseHeader(E);
},getAllResponseHeaders:function(){if(this.__nV){return;
}return this.__sL.getAllResponseHeaders();
},getRequest:function(){return this.__sL;
},dispose:function(){if(this.__nV){return false;
}if(window.detachEvent){window.detachEvent(m,this.__sG);
}try{this.__sL.onreadystatechange;
}catch(G){return;
}var F=function(){};
this.__sL.onreadystatechange=F;
this.__sL.onload=F;
this.__sL.onerror=F;
this.abort(true);
this.__sL=null;
...
},__sL:null,__sJ:null,__sF:null,__sG:null,__sy:null,__sH:null,__sI:null,__eH:null,__nV:null,__sM:null,__sK:null,__sN:function(){this.__sL=this._createNativeXhr();
this.__sL.onreadystatechange=this.__sF;
this.__nV=this.__sH=this.__sI=false;
},__sO:function(){var I=this.__sL,J=true;
...
if(this.readyState===qx.bom.request.Xhr.DONE&&this.__sI&&!this.__sH){return;
}if(!this.__sJ&&(I.readyState==2||I.readyState==3)){return;
}this.status=0;
...
}}},__sP:function(){var L=this;
if(qx.core.Environment.get(s)==p&&qx.core.Environment.get(n)<8){if(this.__sJ&&!this.__sH&&this.readyState>=qx.bom.request.Xhr.LOADING){if(this.readyState==qx.bom.request.Xhr.LOADING){return ;
}
...
this.__sQ();
},__sQ:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__sM);
if(this.__eH){this.ontimeout();
if(qx.core.Environment.get(s)===l){this.onerror();
}this.__eH=false;
}else{this.statusText?this.onload():this.onerror();
}this.onloadend();
}},__sR:function(){var M=this.__sL;
...
}},__sT:function(){if(qx.core.Environment.get(s)==p&&(this.getResponseHeader(a)||r).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var O=new window.ActiveXObject(g);
O.async=false;
O.validateOnParse=false;
O.loadXML(this.responseText);
this.responseXML=O;
}},__sU:function(){try{if(this){this.dispose();
}}catch(e){}},__sV:function(){var name=qx.core.Environment.get(s);
